...
def main():
    filename = "300_300_frontal"
    planet = "Planet300_stable"

    print("Initializing Simulation")
    planet1 = sl.InitialState(sl.InitialState.load_stable_planet(planet + ".dat")).move_center_from_origin(R=[-2e9,0,0]).add_boost(v=[2e4,0,0])
    planet2 = sl.InitialState(sl.InitialState.load_stable_planet(planet + ".dat")).move_center_from_origin(R=[2e9,0,0]).add_boost(v=[-2e4,0,0])
