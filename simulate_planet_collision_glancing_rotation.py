...
def main():
    filename = "600_600_glancing_rotation"
    planet = "Planet600_stable"

    print("Initializing Simulation")
    planet1 = sl.InitialState(sl.InitialState.load_stable_planet(planet + ".dat")).add_rotation([0,0,2 * np.pi / (24 * 3600)]).move_center_from_origin(R=[-2e9,0.9e8,0]).add_boost(v=[2e4,0,0])
    planet2 = sl.InitialState(sl.InitialState.load_stable_planet(planet + ".dat")).add_rotation([0,0,- 2 * np.pi / (24 * 3600)]).move_center_from_origin(R=[2e9,-0.9e8,0]).add_boost(v=[-2e4,0,0])
